...
```mermaid
flowchart TB
    subgraph TANK["액침냉각 탱크 (10.100.230.130~134)"]
        subgraph K8S["Kubernetes v1.35.4 클러스터"]
            N1["node1 — 10.100.230.130<br/>control-plane + Ray head<br/>RTX 5060 Ti 16GB"]
            N2["node2 — 10.100.230.131<br/>GPU worker<br/>RTX 5060 Ti 16GB"]
            N3["node3 — 10.100.230.132<br/>GPU worker<br/>RTX 5060 Ti 16GB"]
            N4["node4 — 10.100.230.133<br/>GPU worker<br/>RTX 5080 16GB"]
            N5["node5 — 10.100.230.134<br/>NPU worker<br/>Furiosa RNGD 48GB HBM"]
        end
    end

    subgraph SERVE["AI 추론 (Ray Serve :31494)"]
    ...
    subgraph PUE["PUE 부하 제어"]
        GPU_LOAD["GPU Load Controller<br/>(node2-4 대상)"]
        NPU_LOAD["NPU Load Generator<br/>(node5 대상)"]
        WD["Inference Watchdog"]
        WEB["Web UI :31600"]
    end

    subgraph MON["모니터링"]
        PROM["Prometheus :9090"]
        VM[("VictoriaMetrics :30171")]
        GRA["Grafana :31618"]
        ALERT["Alertmanager :9093"]
    end

    WS["관리 워크스테이션<br/>10.100.250.103 → ssh newcluster-master"]
...
├── docs_immersion/                   ← 액침냉각 시스템 상세 문서
│   ├── 01_system_overview.md           시스템 전체 개요 + 기술 스택
│   ├── 02_cluster_nodes.md             5노드 상세 사양 + 접속 방법
│   ├── 03_ai_models.md                 5개 AI 모델 + Ray Serve 구조
│   ├── 04_data_collection.md           데이터 수집 채널 + CronJob
│   ├── 05_pue_measurement.md           PUE 부하 제어 + 안전장치
│   ├── 06_monitoring_alerting.md       Grafana + 알림 규칙 + Slack
│   ├── 07_operation_guide.md           운영 가이드
│   ├── 08_deployment_guide.md          배포 절차
│   └── 09_troubleshooting.md           장애 대응
│
...
│
├── src/                              ← Python 소스
│   ├── collectors/                     EDAC/IPMI/SMART 수집기
...
│   ├── build_npu_artifact.py          NPU 아티팩트 빌드
│   └── npu_serve_entrypoint.sh        NPU 서빙 엔트리포인트
│
├── models/checkpoints/               ← 모델 체크포인트 (xgboost_model.json, anomaly_transformer.pt)
└── tests/                            ← pytest 테스트
```
...
- **node1 (10.100.230.130)에서 직접 실행하는 경우**: `kubectl` 바로 사용 가능 (master 노드)
- **외부 워크스테이션에서 실행하는 경우**: `ssh newcluster-master` 설정 필요

### 상태 확인

```bash
# 전체 Pod 상태
kubectl -n failure-prediction get pods                     # node1에서 직접
ssh newcluster-master "kubectl -n failure-prediction get pods"  # 외부에서
