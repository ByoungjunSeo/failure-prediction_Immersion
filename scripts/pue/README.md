...
| 단계 | GPU target | NPU INTERVAL_SEC | NPU 전력 (실측) | 비고 |
|------|-----------|-------------------|-----------------|------|
| 30%  | 30%       | 5.0s              | ~84W            | 저부하 |
| 50%  | 50%       | 3.0s              | ~102W           | 중간 부하 |
| 90%  | 90%       | 0.5s              | ~155W           | 고부하 |
| 99%  | 99%       | 0.2s              | ~156W           | 최대 부하 |
| OFF  | -         | -                 | ~45W (idle)     | |

NPU TDP: ~160W. GPU는 PI 피드백 컨트롤러가 target에 수렴.

**GPU 내부 클램핑**: PI 컨트롤러 내부 안전 제한으로 target이 85%로 클램핑됨.
따라서 `start_90`과 `start_99`는 GPU 측면에서 동일하게 ~88-91%로 동작.
`start_30`(→30%)과 `start_50`(→50%)은 정상 수렴.

**NPU 특성**: NPU는 점유(occupancy) 모델 특성상 90%와 99%의 실측 전력 차이가
거의 없음 (155W vs 156W). `start_90`과 `start_99` 모두 실질적으로 NPU 풀가동.
NPU 부하 수준의 실질적 구분은 30%(84W) / 50%(102W) / 풀가동(155W+) 3단계.

**실질 부하 단계 요약**:
| 스크립트 | GPU 실측 | NPU 실측 |
|----------|---------|---------|
| start_30 | ~30% | 84W (저부하) |
| start_50 | ~50% | 102W (중부하) |
| start_90 | ~88% (85% 클램핑) | 155W (풀가동) |
| start_99 | ~88% (85% 클램핑) | 156W (풀가동) |
